...
const CURRENT_SEASON_CACHE_TTL_MS = IS_PRODUCTION
  ? 7 * 24 * 60 * 60 * 1000
  : 4 * 60 * 60 * 1000;
const HISTORICAL_SEASON_CACHE_TTL_MS = IS_PRODUCTION
  ? 30 * 24 * 60 * 60 * 1000
  : 4 * 60 * 60 * 1000;
...
function getCurrentSeason() {
  return ALL_SEASONS[0] || null;
}

function isCurrentSeason(year, season) {
  const current = getCurrentSeason();
  return current?.year === year && current?.season === season;
}

function getSeasonCacheTtl(year, season) {
  return isCurrentSeason(year, season)
    ? CURRENT_SEASON_CACHE_TTL_MS
    : HISTORICAL_SEASON_CACHE_TTL_MS;
}

function mergeSeasonItem(cached, scraped) {
  return {
    ...cached,
    ...scraped,
    imdb_id: cached.imdb_id,
    title: scraped.title,
    title_english: scraped.title_english || scraped.title,
    score: scraped.score,
...
async function getSeasonData(year, season, sort = "rated", { force = false } = {}) {
  const cacheKey = `${year}-${season}-${sort}`;
  const cached = seasonDataCache.get(cacheKey);
  const now = Date.now();
  const ttl = getSeasonCacheTtl(year, season);

  if (
    !force &&
    cached &&
    now - cached.lastUpdated < ttl &&
  ...
  try {
    ...
    const scraped = await scrapeMalSeasonalPage(year, season);
    ...
    for (const item of scraped) {
      const prev = cachedByMalId.get(item.mal_id);
      if (prev?.imdb_id) {
        existingItems.push(mergeSeasonItem(prev, item));
      } else {
        newItems.push(item);
      }
    }
    ...
    if (newItems.length > 0) {
      console.log(
        `[refresh] ${seasonLabel}: resolving ${newItems.length} new title(s), ${existingItems.length} cached`,
      );
    } else if (isCurrentSeason(year, season)) {
      console.log(
        `[refresh] ${seasonLabel}: ratings refresh (${existingItems.length} titles, scores updated)`,
      );
    }

    const resolvedNew = await enrichSeasonWithImdbIds(newItems, seasonLabel);
    let merged = [...existingItems, ...resolvedNew];

    if (sort === "rated") {
      merged = merged.sort((a, b) => (b.score || -1) - (a.score || -1));
    }

    const finalData = merged.slice(0, 25);
...
function scheduleCurrentSeasonRefresh() {
  ...
  const refresh = async () => {
    ...
    const cacheKey = `${current.year}-${current.season}-rated`;
    seasonDataCache.delete(cacheKey);

    console.log(`[schedule] Weekly rescrape: ${current.label}`);
    await getSeasonData(current.year, current.season, "rated", { force: true }).catch(
      (err) => {
        console.error(`[schedule] Weekly rescrape failed:`, err.message);
      },
    );
  };

  setInterval(refresh, CURRENT_SEASON_CACHE_TTL_MS);
}
